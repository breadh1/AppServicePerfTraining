...
app.MapGet("/start", () =>
{
    ...
    _ = Task.Run(async () =>
    {
        while (!leakCts.Token.IsCancellationRequested)
        {
            try
            {
                // Generate ~20MB of objects per second (4 times, 5MB each, 250ms interval)
                for (int i = 0; i < 4 && !leakCts.Token.IsCancellationRequested; i++)
                {
                    // Create 5MB byte array
                    var leakyData = new byte[5 * 1024 * 1024];
                    ...
                    await Task.Delay(250, leakCts.Token);
                }
            }
    ...
    return "Memory leak started! Generating ~20MB of objects per second...";
});
